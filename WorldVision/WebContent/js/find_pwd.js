...
function numChk(){
    ...
    document.getElementById('verificationCode1').disabled=false;       //인증번호 발송시 인증번호 확인 input칸 활성화
...
//핸드폰번호로 아이디찾기 확인 이벤트
function numCkBtn1() {
	var pwdSerch = document.pwdSerch; //폼테그 담은 변수
	var phoneId = document.getElementById('phoneId1').value;
	...
	if(phoneId==""){
		alert("아이디에 공백은 입력하실 수 없습니다.");
		return pwdSerch.phoneId1.focus();
	}else if (phoneName == "") {
		alert("이름에 공백은 입력하실 수 없습니다.");
		return pwdSerch.phoneName1.focus();
	} else if (phoneNum1 == "") {
		alert("휴대폰번호에 공백은입력하실 수 없습니다.");
		return pwdSerch.phoneNum1.focus();
	} else if (namePattern.test(phoneName) == false) {
		alert("이름 형식과 일치하지 않습니다. \n다시 한번 입력해주세요.");
		return pwdSerch.phoneName.focus();
		...
		if((chkCode==randomNum)){
			pwdSerch.method = "post"; //서버에 값 전송
			pwdSerch.action = "/WorldVision/worldVision.jb?type=find_pwd"; //서버에 값 전송
			pwdSerch.submit(); //서버에 값 전송
		}
	}		
}
...
function numChk2(){
    ...
    if(emailPattern.test(confirm2)==true){                            //휴대폰 번호 형식 확인
        ...
        console.log(randomNum);
        tid=setInterval('msg_time()',1000);
        document.getElementById("hiddenArea").style.display="block";   // 발송버튼 클릭 후 인증번호 확인구역 나타남
...
var chkResult2=false; //이메일로찾기 탭의 인증결과 담은 변수
...
function numCkBtn2() {
	...
	if(phoneId2 == ""){
		alert("아이디에 공백은 입력하실 수 없습니다.");
		return pwdSerch.phoneId2.focus();
	}else if (phoneName2 == "") {
		alert("이름에 공백은 입력하실 수 없습니다.");
		return pwdSerch.phoneName2.focus();
	} else if (phoneNum2 == "") {
		alert("이메일주소에 공백은 입력하실 수 없습니다.");
		return pwdSerch.phoneNum2.focus();
